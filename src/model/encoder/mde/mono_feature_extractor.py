...
class MonoFeatureExtractor(torch.nn.Module):
    def __init__(self, 
        ...
        self.mono_layer_idx  = mono_layer_idx

        # upsample features to the original resolution
        self.feature_upsampler = DPTHead(**model_configs[encoder],
                                        downsample_factor=upsampler_factor,
                                        return_feature=True,
    ...
    def forward(self, images):
        ...
        '''
        TODO: current: layer 11, may be changed later
        '''
        mono_features = self.da_model.pretrained.get_intermediate_layers(
                img_for_dino, [self.mono_layer_idx], return_class_token=False
            )
        depth = self.da_model(img_for_dino)

        if DEBUG:
            print("\033[94m MonoFeatureExtractor mono_features shape, depth shape \033[0m", mono_features[0].shape, depth.shape)
            print(mono_features[0][0][0][:5])
            import matplotlib.pyplot as plt
            fig = plt.figure(figsize=(12, 6))
            ax1 = fig.add_subplot(1, 2, 1)
            ax1.imshow(depth[0].cpu().detach().numpy(), cmap='viridis')
            ax1.set_title('Predicted Depth')
            ax1.axis('off')
            ax2 = fig.add_subplot(1, 2, 2)
            ax2.imshow(depth[1].cpu().detach().numpy(), cmap='viridis')
