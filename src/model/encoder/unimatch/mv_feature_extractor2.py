...
class FeatureExtractor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.shift_scale_head = nn.ModuleList()
        ...
        for i in range(self.num_scales):
            ...
            # regressor residual
            self.regressor_residual.append(nn.Conv2d(32, 1, 1))
            ...
            shift_channels = in_channels - curr_depth_candidates
            self.shift_scale_head.append(
                nn.Sequential(
                    nn.Conv2d(shift_channels, shift_channels // 4, 3, 1, 1, padding_mode="replicate"),
                    nn.GELU(),
                    nn.Conv2d(shift_channels // 4, 2, 3, 1, 1, padding_mode="replicate"),
    ...
    def forward(
        ...
        **kwargs,
        ...
        features_mv = features_list_mv[0]  # [BV, C, H, W]

        # regressor
        features_cnn = features_list_cnn[0]  # [BV, C, H, W]

        features_mono = features_list_mono[0]  # [BV, C, H, W]

        concat = torch.cat(
            (features_cnn, features_mv, features_mono), dim=1
        )

        depth_residual = self.upsampler(
        ...
        depth_residual = self.regressor_residual[0](depth_residual)
        # regress scale and shift
        scale_features = torch.cat(
                (features_cnn, features_mv, features_mono), dim=1
            )
        shift_scale = self.shift_scale_head[-1](scale_features)
        raw_depth = rearrange(raw_depth, "b v h w -> (b v) h w")
        scale = F.softplus(shift_scale[:, 0, :, :])  # [BV, 1, 1]
        shift = 0.1*torch.nn.functional.tanh(shift_scale[:, 1, :, :])  # [BV, 1, 1]
        depth_rescale = raw_depth * scale + shift # [BV, H, W]
        depth_rescale += depth_residual[:, 0]
        depth_rescale = torch.clamp(depth_rescale, min=0.0)  # [BV, H, W]
        
        depth_pred = rearrange(
            depth_rescale, "(b v) ... -> b v ...", b=b, v=v
        )   # [B, V, H, W]

        results_dict.update({"depth_preds": [depth_pred]})
